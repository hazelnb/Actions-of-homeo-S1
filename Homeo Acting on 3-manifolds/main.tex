...
\begin{proof}\fix{outline this proof more clearly}
    ...
    from $\pconf{2}\times F$
    to the $\homeo$ invariant tube $X$\improve{introduce notation},
    which is connected,
    so $\pconf{2}\times F$
...
Of the results proven in the preceding sections of this paper, only \cref{thm:2d-orbit-cpctification} takes into account actions with fixed points. This section presents a brief summary of the theory of $\SO$ manifolds with fixed points and provides a construction for a large family of $\homeo$ actions with fixed-points. 

Note that trivially any global fixed point of a $\homeo$ action is fixed by the rotation subgroup. Moreover, by \cref{prop:orbit-computation}, within a nontrivial $\homeo$ orbit, the rotation subgroup acts without fixed points, so the $\SO$ fixed points are exactly the fixed point of the overall $\homeo$ action. 
